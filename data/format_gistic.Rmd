...
new_gistic_long <- gistic_long_calls %>% mutate(Hugo_Symbol=`Gene Symbol`, Chrom = Cytoband, 
                             Tumor_Sample_Barcode = tcga_id, Start_Position = NA, Reference_Allele = NA,
                             Ncbi_Build = "GRCh38", Tumor_Seq_Allele1=NA, Tumor_Seq_Allele2 = NA, 
                             Dbsnp_Rs = NA, Dbsnp_Val_Status = NA
                             )
...
packageDir:::dataCleaningAndCheck(head(new_gistic_long))

#
packageDir:::addPidColumn(head(new_gistic_long))
```